	...
	{#if viewport_items && viewport_items.length > 0}
		{#each viewport_items as item, index}
			{#if shortcuts && shortcuts.length > index - 1}
				<ListItem {service} {item} selected={index === currentSelection - viewport_top} shortcut={shortcuts[index]} />
			{:else}
				<ListItem {service} {item} selected={index === currentSelection - viewport_top} />
			{/if}
		{/each}
	{/if}
</div>
